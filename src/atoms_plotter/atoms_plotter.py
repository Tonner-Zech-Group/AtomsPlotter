...
from .utils import sliding_window, color_gradient, is_inside_cell
class atoms_plotter(
):
    def __init__(self, 
                ...
                scale=100, # I determined this to be a nice size. adjust if necessary.
                show=True,
                show_bonds=True,
                use_bondorders=True,
        ...
        self.scale=scale # I determined this to be a nice size. adjust if necessary.
        self.show=show
    ...
    def plot_bond(self,xatom1,yatom1,xatom2,yatom2,a1,a2,allx,ally,offset):
        def check_color(list1,list2):
            for n,c in enumerate(list1):
                if c != list2[n]:
                    ...
                    break
            return True
        ZORDER=min([self.atoms.positions[a1][self.view],self.atoms.positions[a2][self.view]])
        colormid=(self.color1+self.color2)/2
        ca1=self.atoms.get_chemical_symbols()[a1]
        ca2=self.atoms.get_chemical_symbols()[a2]
        if self.draw_outline == True:
            OUTLINE = self.outline
        else:
            OUTLINE = None
        if self.bond_gradient == True:
            NUM = 61
        else:
            NUM = 3
        ...
        if np.all(offset == 0):
            ...
            if check_color(self.color1,self.color2) == True:
                self.ax.plot(allx,ally,color=self.color1,zorder=ZORDER,path_effects=[self.outline],linewidth=self.bondlinewidth,solid_capstyle=self.capstyle)
            else:
                color_gradient1=color_gradient(allx,ally,c1=self.color1,c2=self.color2,zorder=ZORDER,linewidth=self.bondlinewidth, capstyle=self.capstyle,outline=OUTLINE, NUM = NUM)
                self.ax.add_collection(color_gradient1)
        else:
            if self.draw_outline == True:
                self.ax.plot(xatom1,yatom1,color='k',zorder=ZORDER,linewidth=self.outline_bonds,solid_capstyle=self.capstyle)
                self.ax.plot(xatom2,yatom2,color='k',zorder=ZORDER,linewidth=self.outline_bonds,solid_capstyle=self.capstyle)
            if ca1 == ca2:NUM=3
            color_gradient1=color_gradient(xatom1,yatom1,self.color1,colormid,zorder=ZORDER,linewidth=self.bondlinewidth, capstyle=self.capstyle,outline=None, NUM = NUM-1)
            self.ax.add_collection(color_gradient1)
            color_gradient2=color_gradient(xatom2,yatom2,self.color2,colormid,zorder=ZORDER,linewidth=self.bondlinewidth, capstyle=self.capstyle,outline=None, NUM = NUM-1)
            self.ax.add_collection(color_gradient2)
    def plot_atoms_2D(self):
        def draw_bonds(self):
                self.bonds()
                if self.colorbonds == False:
                    if self.lewis == True:
                        self.color1=np.array((0,0,0));self.color2=np.array((0,0,0))
                    else:
                        self.color1=np.array((0.8,0.8,0.8));self.color2=np.array((0.8,0.8,0.8))

                if isinstance(self.colorbonds, str):
                    self.color1=np.array((float(self.colorbonds),float(self.colorbonds),float(self.colorbonds)))
                ...
                for a1,a2,offset,bondorder,bondorderoffset in self.bondatoms:
                    if np.all(offset == 0):
                        ...
                        self.outline=mpe.withStroke(linewidth=self.outline_bonds, foreground='black',capstyle='butt')
                    else:
                        self.capstyle='projecting'
                        self.outline_capstyle='projecting'
                        if self.draw_outline == True:self.outline=mpe.withStroke(linewidth=self.outline_bonds, foreground='black',capstyle='projecting')
                        if self.draw_outline == False:self.outline=None
                        
                    if self.colorbonds == True and type(self.colorbonds) != str:
                        self.color1=np.array(self.COLORS[a1]);self.color2=np.array(self.COLORS[a2]);
                    if bondorder==1:
                        ...
                        self.plot_bond(xatom1,yatom1,xatom2,yatom2,a1,a2,allx,ally,offset=offset)
                    if bondorder==2:
        ...
        if self.show_unit_cell == True:
            self.ax.plot(self.uXs,self.uYs,linestyle=self.unit_cell_linestyle,lw=self.bondlinewidth/2,color='black',zorder=100,dash_capstyle='round',dash_joinstyle='round')#zorder=10)
        self.COLORS=[]
        self.sizes=[]
        ...
        self.new_atoms.positions=[[x+self.real_space_shift[0],y+self.real_space_shift[1],z] for x,y,z in self.atoms.positions]
        ...
        for n,a in enumerate(self.atoms.get_atomic_numbers()):
            if self.color_dict == None:
                ...
                if self.plot_atom_colorscaling == True:
                    colorval=colorval*self.atoms[n].position[-1]/(np.linalg.norm(np.amax(self.atoms.positions[:,2])))
                self.COLORS.append(colorval)
            else:
                self.COLORS=[self.color_dict[s] if s in self.color_dict else colors.jmol_colors[a] for s in self.atoms.get_chemical_symbols()]
            if self.lewis == True:
                self.COLORS =[(0,0,0) for s in self.atoms.get_chemical_symbols()]
            sizeval=covalent_radii[a]*self.scale#/np.amax(self.XARRAY)
            self.sizes.append(sizeval)
            if self.ATOMS ==True:
                if self.draw_outline == True:
                    self.ax.scatter(Xs[n],Ys[n],color=self.COLORS[n],s=self.sizes[n],linewidth=outline_atoms,zorder=self.atoms.positions[n][self.view]+0.1,edgecolors='black')#,zorder=10) #plot the atoms.
                else:
                    self.ax.scatter(Xs[n],Ys[n],color=self.COLORS[n],s=self.sizes[n],zorder=self.atoms.positions[n][self.view]+0.1)
        if self.constraints == True:
            if self.atoms.constraints:
                for c in self.atoms.constraints[0].index:
                    if self.ATOMS ==True:
                        self.ax.scatter(Xs[c],Ys[c],color=(0,0,0),marker='x',s=self.sizes[c]/(np.sqrt(math.pi)),zorder=self.atoms[c].position[self.view]+0.11,alpha=0.8,linewidths=1)
    ...
    def plot_atoms_3D(self):
            def draw_bonds(self):
                ...
                if self.draw_outline == True:self.outline=mpe.withStroke(linewidth=self.outline_width, foreground='black',capstyle='butt')
                if self.draw_outline == False:self.outline=None
                if self.colorbonds == False:
                    color1=(0.8,0.8,0.8);color2=(0.8,0.8,0.8)
                    if self.lewis == True:
                        color1=(0,0,0);color2=(0,0,0)
                if isinstance(self.colorbonds, str):
                    color1=(float(self.colorbonds),float(self.colorbonds),float(self.colorbonds))
                    color2=color1
                for a1,a2,offset,bondorder,bondorderoffset in self.bondatoms:
                    if bondorder == 1:
                    ...
                    if bondorder==2:
                        ...
                        for sign in [1,-1]:
                            ...
                            yatom2=[atompos2[1],midb[1]]
                            zatom2=[atompos2[2],midb[2]]
                            if self.plot_color_gradient == True:
                                self.plot_colorline(xatom1,yatom1,xatom2,yatom2,a1,a2)
                            else:
                                self.plot_line(xatom1,yatom1,xatom2,yatom2,a1,a2)
                    if bondorder==3:
                        b=np.array(bondorderoffset)
                        for sign in [1,0,-1]:
                            atompos1=self.atoms[a1].position+b*sign
                    ...
                    if type(self.colorbonds) != float and self.colorbonds ==True:
                        color1=self.COLORS[a1];color2=self.COLORS[a2];
                    if self.draw_outline == True:
                        self.ax.plot(xatom1,yatom1,zatom1,color=color1,zorder=atompos1[self.view],linewidth=self.bondlinewidth,path_effects=[self.outline],solid_capstyle='butt')
                        self.ax.plot(xatom2,yatom2,zatom2,color=color2,zorder=atompos2[self.view],linewidth=self.bondlinewidth,path_effects=[self.outline],solid_capstyle='butt')
                    else:
                        self.ax.plot(xatom1,yatom1,zatom1,color=color1,zorder=atompos1[self.view],linewidth=self.bondlinewidth,solid_capstyle='round')
                        self.ax.plot(xatom2,yatom2,zatom2,color=color2,zorder=atompos2[self.view],linewidth=self.bondlinewidth,solid_capstyle='round')
            ...
            for x,y,z in zip(X,Y,Z):
                ...
                uZs.append(mult[2])
            if self.show_unit_cell == True:
                self.ax.plot(uXs,uYs,uZs,linestyle='dashed',lw=self.bondlinewidth,color='black',zorder=0)
            ...
            for n,a in enumerate(self.atoms.get_atomic_numbers()):
                if self.color_dict == None:
                    ...
                    self.COLORS=[self.color_dict[s] for s in self.atoms.get_chemical_symbols()]
                if self.lewis == True:
                    self.COLORS=[(0,0,0) for s in self.atoms.get_chemical_symbols()]
                sizeval=covalent_radii[a]*self.scale#/np.amax(self.XARRAY) 
                self.sizes.append(sizeval)
...
#            self.ax.set_zlim([uZs[0],uZs[6]])
            ...
            for n,a in enumerate(self.atoms):
                if self.ATOMS ==True:
                    self.ax.scatter(Xs[n],Ys[n],Zs[n],color=self.COLORS[n],s=self.sizes[n],zorder=a.position[self.view]+0.1,linewidth=outline_atoms,edgecolors='black',alpha=1)
            if self.ATOMS ==True:
                if self.atoms.constraints and self.constraints == True:
                    for c in self.atoms.constraints[0].index:
                        self.ax.scatter(Xs[c],Ys[c],Zs[c],color=(0,0,0),s=self.sizes[c]/(np.sqrt(math.pi)),marker='x',zorder=self.atoms[c].position[self.view]+0.11,alpha=0.5)
    def plot(self):
        #if self.show == True:
        #    mpl.use('TkAgg')
        #else:
